...
const listPaymentOptions = {
  ...
  js_client: (config, sessionId) => {
    if (config.type === 'credit_card') {
      ...
      let onloadFunction = `window.pagseguroBrand=function(a){return new Promise(function(b,c){PagSeguroDirectPayment.setSessionId("${sessionId}"),PagSeguroDirectPayment.getBrand({cardBin:a.number.substring(0,6),success:function(a){b(a.brand)},error:function(a){c(a)}})})},window.pagseguroHash=function(a){return new Promise(function(b,c){console.log("Credicard Data",a),PagSeguroDirectPayment.setSessionId(""),PagSeguroDirectPayment.getBrand({cardBin:a.number.replace(/\s/g,"").substring(0,6),success:function(d){console.log("Brand",d);let e={cardNumber:a.number,brand:d.brand.name,cvv:a.cvc,expirationMonth:a.month,expirationYear:a.year,success:function(a){b(a)},error:function(a){c(a)}};console.log("Card",e),PagSeguroDirectPayment.createCardToken(e)},error:function(a){c(a)}})})};`
      return {
        cc_brand: {
