      ...
      .then(transaction => {
        ...
        return getTransaction(transaction.code)
          .then(async result => {
            ...
            let order = await appSdk.apiRequest(storeId, resource, method).catch(() => logger.log('transacation_code not found, waiting 30s before try again'))
            if (order) {
              ...
              setTimeout(() => {
                logger.log('Looking for order number', transaction.reference)
