...
module.exports = (appSdk, notificationCode) => {
  ...
  const updateTransaction = (notificationCode) => {
    ...
    return ps.notification.getNotification(notificationCode)
      ...
      .then(transaction => {
        ...
        return getTransaction(transaction.code)
          .then(async result => {
            ...
            const resource = `orders.json?transactions.intermediator.transaction_code=${transaction.code}` +
              '&fields=_id,number,transactions._id,transactions.intermediator.transaction_code,financial_status'
            const method = 'GET'
            storeId = result.transaction_store_id
            const order = await appSdk.apiRequest(storeId, resource, method)
              .then(resp => resp.response.data.result[0])

            if (order && order.transactions) {
              const orderTransactions = order.transactions
                .find(trans => trans.intermediator.transaction_code === transaction.code)
      ...
      .catch(err => {
        err.notificationCode = notificationCode
        err.storeId = storeId
        logger.error(err)
      })
  }
