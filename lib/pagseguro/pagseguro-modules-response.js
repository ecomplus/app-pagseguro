  ...
  return {
    ...
    'transaction': {
      ...
      'creditor_fees': {
        ...
        'intermediation':Number(Math.abs(payload.transaction.feeAmount).toFixed(2))
      },
      'currency_id': 'BRL',
      'installments': {
        'number': Number(payload.transaction.installmentCount),
        'tax': (payload.transaction.feeAmount > 0),
        'total': Number(payload.transaction.grossAmount),
        'value': Number(Math.abs(payload.transaction.feeAmount / payload.transaction.installmentCount).toFixed(2))
      },
      'intermediator': {
        'payment_method': {
          'code': 'credit_card',
