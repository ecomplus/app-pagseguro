...
const PAGSEGURO_API_PATH = (process.env.PS_APP_SANDBOX && process.env.PS_APP_SANDBOX === 'true')
...
class Transaction {
  ...
  new(params) {
    ...
    this.client.payment = {
      payment: {
        ...
        extraAmount: parseFloat(-params.amount.discount).toFixed(2)
      }
    }
  ...
  getByCode(transactionCode) {
    const resource = `${PAGSEGURO_API_PATH}/v3/transactions/${transactionCode}` +
...
class Checkout {
  ...
  async card() {
    ...
    const installmentsNumber = this.client.params.installments_number
    const amountTotal = parseInt(this.client.params.amount.total * 1000, 10) / 1000

    if (installmentsNumber > 1) {
      let installmentOptions
      try {
        const session = await this.client.session.new()
        if (session) {
          installmentOptions = await this.client.installments.getInstallments(session, amountTotal)
        }
      } catch (e) {
        // ignore
      }
    ...
    if (!this.client.installmentsValue || !this.client.installmentsValue.value) {
    ...
    this.client.payment.payment = {
      ...
      creditCard: {
        ...
        installment: {
          ...
          value: parseFloat(
            (this.client.installmentsValue && this.client.installmentsValue.value) ||
            amountTotal
          ).toFixed(2)
        },
        holder: {
          ...
          birthDate: this.convertDate(
            this.client.params.buyer.birth_date.day,
            this.client.params.buyer.birth_date.month,
            this.client.params.buyer.birth_date.year
          ),
          phone: {
...
class Pay {
  ...
  new() {
    return new Promise((resolve, reject) => {
      ...
      const resource = `${PAGSEGURO_API_PATH}/v2/transactions` +
        `?appId=${appId}&appKey=${appKey}&authorizationCode=${authorizationCode}`
      ...
      axios(options)
        ...
        .then(async data => {
          ...
          const schema = await schemas(this.client.payWith)
...
class Notification {
  ...
  getNotification(notificationCode) {
    const appId = this.client.appId
    const appKey = this.client.appKey
    const resource = `${PAGSEGURO_API_PATH}/v3/transactions/notifications/${notificationCode}` +
