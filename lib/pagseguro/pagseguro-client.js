...
class Checkout {
  ...
  async card () {
    ...
    if (installmentsNumber) {
      ...
      if (!this.client.installmentsValue || !this.client.installmentsValue.total) {
        if (hashs[2]) {
          try {
...
class Installments {
  ...
  getInstallments(session, amount, brand = 'visa') {
    return new Promise((resolve, reject) => {
      ...
      const resource = `https://${env}pagseguro.uol.com.br/checkout/v2/installments.json?sessionId=${session}&amount=${parseFloat(amount).toFixed(2)}&creditCardBrand=${brand}`
